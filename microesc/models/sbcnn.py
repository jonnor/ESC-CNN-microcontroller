...
def build_model(frames=128, bands=128, channels=1, num_labels=10, kernel=(5,5), pool=(4,2), dropout=0.5, depthwise_separable=True):
    """
    ...
    model.add(Convolution2D(48, kernel, padding='same', input_shape=(bands, frames, channels)))
    ...
    model.add(Conv2(48, kernel, padding='same', depth_multiplier=2))
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=pool))
    model.add(Activation('relu'))
    ...
    model.add(Conv2(48, kernel, padding='valid', depth_multiplier=2))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
